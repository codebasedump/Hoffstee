window.onload = function() {
    ...
    let _ = document.querySelector("#preloader");
    _ && window.addEventListener("load", () => {
        _.remove()
    });
    ...
    function i() {
        (o.classList.contains("scroll-up-sticky") || o.classList.contains("sticky-top") || o.classList.contains("fixed-top")) && (window.scrollY > 100 ? s.classList.add("scrolled") : s.classList.remove("scrolled"))
    }
    document.addEventListener("scroll", i), window.addEventListener("load", i);
    let n = 0;
    window.addEventListener("scroll", function() {
        if (!o.classList.contains("scroll-up-sticky")) return;
        let e = window.pageYOffset || document.documentElement.scrollTop;
        e > n && e > o.offsetHeight ? (o.style.setProperty("position", "sticky", "important"), o.style.top = `-${header.offsetHeight+50}px`) : e > o.offsetHeight ? (o.style.setProperty("position", "sticky", "important"), o.style.top = "0") : (o.style.removeProperty("top"), o.style.removeProperty("position")), n = e
...
document.addEventListener("mousemove", e => {
    images.forEach(t => {
        let _ = e.clientX / window.innerWidth,
            s = e.clientY / window.innerHeight;
        t.style.transform = `translateX(${(_-.5)*100}px) translateY(${(s-.5)*50}px)`
    })
...
function startVoice() {
    ...
    e.lang = "en-US", e.interimResults = !1, e.maxAlternatives = 1, e.onresult = function(e) {
        let t = e.results[0][0].transcript.toLowerCase(),
            _ = !1;
        for (let s in pages)
            if (t.includes(s)) {
                _ = !0, speak(`Navigating to ${s} page`), window.location.href = pages[s];
                break
            } _ || speak("Sorry, I couldn't find that page.")
...
function speak(e) {
    ...
    t.lang = "en-US", window.speechSynthesis.speak(t)
}
...
window.addEventListener("load", aosInit), $(document).ready(function() {
